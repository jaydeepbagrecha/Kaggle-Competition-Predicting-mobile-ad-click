...
submission = 'submission_own.csv'  # path of to be outputted submission file

# B, model
alpha = .1  # learning rate
beta = 1.   # smoothing parameter for adaptive learning rate
L1 = 0.0004     # L1 regularization, larger value means more regularized
L2 = 0.00004     # L2 regularization, larger value means more regularized
...
def data(path, D):
    ...
    for t, row in enumerate(DictReader(open(path))):
        ...
        # add resolution as a new feature
        size_col = ['C15', 'C16']
        index = abs(hash( row[size_col[0]] + '_x_' + row[size_col[1]] )) % D
        x.append(index)

        # add day as a new feature
        x.append(dayofweek[str(date)])
